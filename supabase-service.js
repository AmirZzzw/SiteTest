...
// تابع همگام‌سازی سفارشات از localStorage به Supabase
async function syncLocalOrdersToSupabase() {
    try {
        ...
        const unsyncedOrders = localOrders.filter(order => !order.supabase_id);
        
        if (unsyncedOrders.length === 0) {
            console.log('✅ All orders are already synced');
        ...
        for (const order of unsyncedOrders) {
            try {
                ...
                if (error) {
                    console.warn(`⚠️ Failed to sync order ${order.id}:`, error.message);
                } else {
                    console.log(`✅ Synced order ${order.id} to Supabase ID: ${data.id}`);
                    
                    // آپدیت localStorage
                    const updatedOrders = localOrders.map(o => {
                        if (o.id === order.id) {
                            return { ...o, supabase_id: data.id };
                        }
                        return o;
                    });
                ...
                console.warn(`⚠️ Error syncing order ${order.id}:`, orderError.message);
            }
        }
        
        console.log(`🎉 Sync completed: ${syncedCount}/${unsyncedOrders.length} orders synced`);
...
window.syncLocalOrdersToSupabase = syncLocalOrdersToSupabase;

async function createNewOrder(orderData) {
    try {
        console.log('🚀 Creating order in Supabase (full)...');
        
        // 1. کاهش حجم تصویر قبل از ذخیره
        let compressedImage = null;
        if (orderData.receipt?.image) {
            compressedImage = await compressImage(orderData.receipt.image);
            console.log('📸 Image compressed for Supabase');
        }
        
        // 2. ساختار داده کامل برای Supabase
        const orderToSave = {
            ...
            receipt_info: {
                ...
                fileSize: orderData.receipt?.fileSize || 0,
                fileType: orderData.receipt?.fileType || '',
                note: orderData.receipt?.note || '',
                image: compressedImage, // تصویر فشرده
                status: 'در انتظار تأیید',
        ...
        if (error) {
            ...
            // اگر خطای حجم بود، دوباره بدون عکس تلاش کن
            if (error.message.includes('payload too large') || error.code === '413') {
                ...
                const orderWithoutImage = {
                    ...orderToSave,
                    receipt_info: {
                        ...orderToSave.receipt_info,
                        image: null,
                        note: orderToSave.receipt_info.note + ' (تصویر به دلیل حجم بالا ذخیره نشد)'
                    }
                };
                
                const { data: data2, error: error2 } = await supabase
                    .from('orders')
                    .insert([orderWithoutImage])
                    .select()
                    .single();
        ...
        console.log('✅ Order saved to Supabase, ID:', data.id);
        
        // 4. ذخیره ID در localStorage برای دسترسی سریع
        const localBackup = {
            id: data.id,
            user_phone: orderToSave.user_phone,
            total: orderToSave.total,
            status: orderToSave.status,
            created_at: orderToSave.created_at
        };
        
        const ordersBackup = JSON.parse(localStorage.getItem('sidka_orders_backup') || '[]');
        ordersBackup.push(localBackup);
        localStorage.setItem('sidka_orders_backup', JSON.stringify(ordersBackup));
        ...
        return {
            ...
            order: data,
            supabaseId: data.id,
            message: 'سفارش با موفقیت در سرور ذخیره شد'
        };
        
    } catch (error) {
        console.error('❌ Fatal error in createNewOrder:', error);
        
        // حالت fallback: ذخیره در localStorage
        return await createOrderFallback(orderData);
    }
}
...
async function compressImage(base64Image) {
    return new Promise((resolve) => {
        ...
        img.onload = function() {
            ...
            // کیفیت 70% برای تعادل بین حجم و کیفیت
...
// حالت fallback
async function createOrderFallback(orderData) {
    try {
        ...
        const order = {
            ...
            user_phone: orderData.customerInfo?.phone || orderData.userId,
            total: orderData.total || 0,
            status: 'در انتظار تأیید (آفلاین)',
            customer_info: orderData.customerInfo || {},
            receipt_info: {
                fileName: orderData.receipt?.fileName || '',
                note: orderData.receipt?.note || '',
                status: 'آفلاین - نیاز به sync'
            },
            items: orderData.items || [],
            created_at: new Date().toISOString(),
            is_fallback: true
        ...
        const orders = JSON.parse(localStorage.getItem('sidka_orders_fallback') || '[]');
        orders.push(order);
        localStorage.setItem('sidka_orders_fallback', JSON.stringify(orders));
        ...
        return {
            ...
            order: order,
            isFallback: true,
            message: 'سفارش به صورت آفلاین ذخیره شد. بعداً sync خواهد شد.'
        };
        ...
        return {
            success: false,
            error: 'سیستم ذخیره‌سازی در دسترس نیست'
        };
    }
}
// ====== تابع فشرده‌سازی برای localStorage ======
function compressImageForLocalStorage(base64Image) {
    return new Promise((resolve) => {
        const img = new Image();
        img.onload = function() {
            const canvas = document.createElement('canvas');
            ...
            // اندازه بسیار کوچک برای صرفه‌جویی در فضا
            const maxWidth = 400;
            const maxHeight = 400;
            ...
            // فشرده‌سازی شدید
            const compressedBase64 = canvas.toDataURL('image/jpeg', 0.4); // کیفیت 40%
            resolve(compressedBase64);
        };
        img.src = base64Image;
    });
...
// ====== تابع حذف سفارشات رد شده از localStorage ======
function cleanupLocalStorageOrders() {
    try {
        let orders = JSON.parse(localStorage.getItem('sidka_orders') || '[]');
        const initialCount = orders.length;
        
        // فقط سفارشات "در انتظار تأیید" و "تأیید شده" رو نگه دار
        orders = orders.filter(order => 
            order.status === 'در انتظار تأیید' || 
            order.status === 'تأیید شده'
        );
        
        // همچنین سفارشات قدیمی (بیش از ۷ روز) رو حذف کن
        const sevenDaysAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
        orders = orders.filter(order => {
            const orderDate = new Date(order.created_at || order.createdAt || 0).getTime();
            return orderDate > sevenDaysAgo;
        });
        
        // حداکثر ۱۵ سفارش نگه دار
        if (orders.length > 15) {
        ...
        if (orders.length < initialCount) {
            ...
            console.log(`🗑️ Cleaned up ${initialCount - orders.length} orders from localStorage`);
        }
...
async function getOrderReceipt(orderId) {
    try {
        ...
        if (supabase) {
            try {
                ...
                if (!error && data && data.receipt_info) {
                    console.log('✅ Receipt loaded from Supabase');
                    return {
                        success: true,
        ...
        // اگر Supabase خطا داد یا وصل نیست، از localStorage بگیر
        const orders = JSON.parse(localStorage.getItem('sidka_orders') || '[]');
        const order = orders.find(o => o.id == orderId || o.supabase_id == orderId);
        
        if (order && order.receipt_info) {
            console.log('✅ Receipt loaded from localStorage');
            return {
                success: true,
...
async function getAllOrders() {
    try {
        ...
        console.log(`✅ Retrieved ${orders?.length || 0} orders from Supabase`);
        
        // 2. گرفتن سفارشات fallback از localStorage
        const fallbackOrders = JSON.parse(localStorage.getItem('sidka_orders_fallback') || '[]');
        ...
        // حالت fallback: فقط از localStorage
        const fallbackOrders = JSON.parse(localStorage.getItem('sidka_orders_fallback') || '[]');
        const backupOrders = JSON.parse(localStorage.getItem('sidka_orders_backup') || '[]');
        
        const allLocalOrders = [...fallbackOrders, ...backupOrders];
        
        return {
            success: true,
            orders: allLocalOrders,
            total: allLocalOrders.length,
            warning: 'دریافت از Supabase ناموفق بود. نمایش داده‌های محلی'
        };
    }
}
...
async function updateOrderStatus(orderId, status) {
    try {
        ...
        const orders = JSON.parse(localStorage.getItem('sidka_orders') || '[]');
        const orderIndex = orders.findIndex(o => o.id == orderId || o.supabase_id == orderId);
        
        if (orderIndex !== -1) {
            if (status === 'رد شده') {
                // اگر سفارش رد شده، از localStorage حذفش کن
                orders.splice(orderIndex, 1);
                console.log(`🗑️ Order ${orderId} removed from localStorage (rejected)`);
            } else {
                // در غیر این صورت آپدیت کن
        ...
        if (supabase) {
            try {
                ...
                if (!error) {
                    ...
                    // اگر رد شده، تصویرش رو از Supabase هم حذف کن (اختیاری)
                    if (status === 'رد شده') {
                        setTimeout(async () => {
                            try {
                                await supabase
                                    .from('orders')
                                    .update({ 
                                        receipt_info: null // یا receipt_info: {deleted: true}
                                    })
                                    .eq('id', orderId);
                                console.log(`🗑️ Receipt image cleared from Supabase for order ${orderId}`);
                            } catch (deleteError) {
                                console.warn('⚠️ Could not clear receipt from Supabase:', deleteError);
                            }
                        }, 1000);
                    }
                }
...
async function getAllTickets() {
    try {
        ...
        try {
            // کوئری ساده‌تر بدون JOIN (چون رابطه وجود نداره)
            const { data, error } = await supabase
                .from('tickets')
                .select('*')
                .order('created_at', { ascending: false });
            ...
            if (error) {
                ...
                // حالت fallback: فقط از localStorage بگیر
                const localTickets = JSON.parse(localStorage.getItem('sidka_tickets') || '[]');
                return { 
                    success: true, 
                    tickets: localTickets,
                    warning: 'خطا در دریافت از Supabase، نمایش داده‌های محلی'
                };
            }
            
            console.log(`✅ Retrieved ${data?.length || 0} tickets from Supabase`);
            
            // داده‌های localStorage رو هم بگیر
            const localTickets = JSON.parse(localStorage.getItem('sidka_tickets') || '[]');
        ...
        console.error('❌ Fatal error in getAllTickets:', error);
